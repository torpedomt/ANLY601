...
##### Q0&Q1
```{r}
df = read.csv('/Users/zmt/Desktop/Assignment3/kernel_regression_2.csv')
...
## Question 2
#### Part 1
(a) Yes. The goal of t-SNE is to maintain the local structure of data, so the distance between points after mapping somewhat indicates their distance before mapping.\
(b) Perplexity is a guess about the number of close neighbors each point has. It is used to balance attention between local and global aspects of your data.\
(c) If the number of steps is too small, then t-SNE will end before it converges. From the article, we know if you see a t-SNE plot with strange “pinched” shapes, chances are the process was stopped too early. Unfortunately, there’s no fixed number of steps that yields a stable result, which is because different data requires different numbers of iterations to converge.\
(d) There are two examples listed in the article. One of the simplest topological properties is containment. t-SNE with low perplexity value greatly exaggerates the size of the smaller group of points. Another is trefoil knot, low perplexity values failed to show global connectivity.
...
(e)
Here we run eta=10 and eta=100 at perplexity=160. (since eta=200 is run before). Larger learning rate will lead to more dispersed clustering result for this dataset.
